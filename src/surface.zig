...
pub const Surface = struct {
    ...
    pub fn drawEnemies(
        ...
        render_info_lookup: []const DenseRenderInfo,
        colour_lookup: []const Colour,
        ...
        for (enemy_states) |enemy_state| {
            const v = map.getVisibility(enemy_state.pos);
            if (v == .Visible) {
                const ax = player_sprite_pos.x + enemy_state.pos.x * sprite_dim_pixels.width;
                const bx = player_pos.x * sprite_dim_pixels.width;
                const ay = player_sprite_pos.y + enemy_state.pos.y * sprite_dim_pixels.height;
                const by = player_pos.y * sprite_dim_pixels.height;
                if (ax >= bx and ay >= by) {
                    const x_idx = player_sprite_pos.x + enemy_state.pos.x * sprite_dim_pixels.width - player_pos.x * sprite_dim_pixels.width;
                    const y_idx = player_sprite_pos.y + enemy_state.pos.y * sprite_dim_pixels.height - player_pos.y * sprite_dim_pixels.height;
                    if (x_idx + sprite_dim_pixels.width < self.width_pixels and y_idx + sprite_dim_pixels.height < self.height_pixels) {
                        self.drawDense(
                            render_info_lookup[@intFromEnum(enemy_state.type)],
                            .{ .x = x_idx, .y = y_idx },
                            clipping_rect,
                            scale,
                            colour_lookup[@intFromEnum(enemy_state.race)],
                        );
                    }
                }
            }
